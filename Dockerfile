...
MAINTAINER Constane Kalivas <looselyrigorous@tolabaki.gr>
...
RUN	cd /tmp/									&&	\
	...
	git clone https://github.com/ut0mt8/nginx-rtmp-module.git -b ${NGINX_RTMP_VERSION}
...
RUN  \
		...
		--enable-openssl \
		--enable-libfdk_aac \
		--enable-libkvazaar --extra-libs=-lpthread \
		--enable-postproc \
		--enable-small \
		--enable-version3 \
...
COPY --from=nginx_builder /tmp/nginx-rtmp-module/stat.xsl /opt/nginx/conf/stat.xsl
